...
iPhone の「ヘルスケア」には、Garmin Connect も Nike Run Club も Apple Watch も書き込みます。
**ここを読めば、Strava もそのアカウントも要りません。** ただしブラウザからは読めないため、
ネイティブアプリ（Capacitor など）が要ります。

実機で確認した範囲では、Garmin Connect は**ワークアウトとして**（種目「ランニング」・距離・
時間・消費カロリー）書き込みます。ラップまで入るかは、実際に読むまで分かりません。
**ラップは「あれば使う」前提で設計しています。** 無くても、距離・時間・ペース・心拍があれば
週の積み上げも強度の判定も成り立ちます。ポイント練習の中身だけ、スクリーンショットで補います。
...
639 件のテストが、痛み検知・走行指示の検査・目標からのペースとVDOTの導出・心拍ゾーンの計算・
