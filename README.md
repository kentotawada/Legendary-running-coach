...
**カルテから開いた画面を閉じたら、カルテに戻ります。** 閉じると全部消えるのは、
開いた道を覚えていないのと同じです。連携・練習の中身・アカウントの各画面には、
左上に「‹ カルテ」が出ます（`Sheet` の `onBack`）。Esc も「1つ戻る」です。
× は今までどおり全部閉じます。
...
選択肢を押すと、**その道具から記録を取り出す道**が出ます。順番が大事です。

1. **記録ファイルで送る**（いちばん詳しい）— 書き出せるか、どこを探すか、どの形式か
2. **スクリーンショットで送る**（どのアプリでも）— ここへ降りれば必ず届く
3. Strava につなぐ（連携が使える環境でだけ）

**確かめていない画面の名前は書きません。**「設定 → ○○ を押す」と書いて実際に無いと、
その人はそこで詰まって諦めます。押す場所ではなく「どこを探すか」を書きます
（Garmin だけは実機で確認済みなので、手順そのものを置いています）。

**書き出せないサービスには、無いと言います。**
Nike Run Club と Apple Watch の純正ワークアウトには書き出しがありません。
探させて時間を無駄にさせるより、スクリーンショットへ案内するほうが早い。
...
| 書き出したファイル（FIT / GPX / TCX） | 実装済み | 無料 | **なし** |
...
- **形式は名前ではなく中身で見分けます。** 端末やアプリが拡張子を落とすことがあるためです
  （FIT は8バイト目の `.FIT`、zip は先頭の `PK`）
- **読めなかった時は、ファイル名と理由をそのまま出します。**
  「3件は読めませんでした」だけでは、どれをどう直せばよいか分かりません
- **記録は走った日の順に並べます。** 追加した順に並べると、古いファイルを後から取り込んだ
  瞬間に順番が崩れます。カルテの見た目だけの問題ではなく、**コーチにも同じ順で渡る**ため、
  直近の練習を取り違えたまま指導することになります
- **すでにある練習に、より詳しい記録が来たら差し替えます。** チャットで話した練習を後から
  ファイルで取り込むと、同じ練習として弾かれますが、中身は後から来たほうが詳しい。
  弾くと区間も推移も永久に失われます。**本人の言葉（felt・主観強度）は必ず残します**
...
- TCX はラップを合計し、**心拍とピッチはラップの長さで重みを付けて**平均します
  （単純平均だと短い1本に引きずられます）。
  要約にピッチを書かない時計があるので、**無い項目は1点ずつの記録から補います**
- GPX は座標から距離を積み、標高は**上った分だけ**を、1m 未満のぶれを捨てて数えます
- **FIT なら、上下動・接地時間・左右バランス・パワーまで入ります**（下の節）
- **zip のままでも開けます**（`src/lib/zip.ts`）。Garmin の「ファイルのエクスポート」は zip で
  降ってきますし、アカウント全体の一括書き出しも zip です。**開ければ1回の操作で全期間が入ります。**
  zip の中の zip も1段だけ開き、写真や設定ファイルは無視します
- 多い時は100件ずつに分けて送ります。途中で失敗しても、そこまで入った分は残ります
- 入力欄の「＋」→「時計の記録を送る」からも同じことができます。
  **スクリーンショットの道は、そのすぐ下に残してあります**
- **スマホの Garmin Connect アプリでは書き出せません**（実機で確認）。
  活動画面の「⋮」にあるのは編集・ギア・お気に入り・削除だけで、「共有」もリンクを送る機能です。
  そこで、連携画面にブラウザ版への入口と歯車までの手順を置いています。
  **深い URL は案内しません。** 認証が要るページを直に指すと、
  ログインしていない人にはログイン画面しか出ず、「押したのに進めない」になるためです
...
**項目ごとに小さなグラフを縦に並べ、横軸だけを共有します**（`src/components/RunCharts.tsx`）。
ペース・心拍・ピッチ・パワー・上下動・接地時間。横軸は距離と時間を切り替えられます。

- **1つのグラフに軸は1つ。** 心拍とペースを重ねると、無い相関が見えてしまいます
- **触った位置は全部のグラフで揃います。**「5km地点で心拍が上がった時、ピッチはどうだったか」は、
  同じ瞬間を横断して見ないと答えられません
- 軸を上下ひっくり返すのは**ペースだけ**です。「速い方が上」は走る人の共通了解なので。
  上下動や接地時間まで裏返すと、**数値が増えたのに線が下がる**ことになり、
  時計の画面と見比べた時に必ず混乱します
- **ピッチは spm に揃えてから残します。** 片脚の回転数（約90）のまま描くと、
  グラフだけが時計の半分の値になります
- 1項目1本なので凡例は置きません。見出しがその名前になっています
- 色は1色（`--chart-ink`）です。項目の区別は見出しが持つので、色で分ける必要がありません
  （明暗・彩度・コントラストは検証スクリプトで light / dark 両方 PASS）

棒の長さが速さ、右に区間ペースと心拍。心拍の推移は距離を横軸に描きます
（時間だと、止まっていた時間で形が歪むため）。

### FIT ファイル（`src/lib/fit.ts`）
...
独自の二進形式なので、自分で解いています（外部ライブラリは入れません）。

**この層でいちばん危ないのは、項目番号の取り違えです。**
FIT は「何番の項目が何を意味するか」を規格の表で決めています。1つずれると、
接地時間のつもりで歩幅が入る、といった事故が起きます。**数値自体は正しく見えるので、
後から気づけません。** そこで二重に守っています。

1. 取り出した値を、走りとして現実的な範囲で検める（`PLAUSIBLE`）。
   外れた値は埋めずに捨てる（上下動 3〜20cm、接地 120〜450ms、左右 35〜65% など）
2. 取り込み口（`/api/import`）でも、もう一度同じ範囲で検める
3. **区間ごとの数値は、ラップの要約ではなく1秒ごとの記録から自分で計算する。**
   record の項目番号は安定しているため

読む側の検証には、**FIT を書く側をテストの中に用意しています**（`tests/fit.test.ts`）。
本物の Garmin のファイルはこの環境で手に入りませんが、規格どおりに組み立てたものを
読ませれば、項目番号・単位・倍率の取り違えはそこで捕まります。

**コーチには「一般的な良い数値」を当てはめさせません。** 上下動も接地時間も歩幅も、
身長・脚の長さ・走る速度で基準が変わります。見るのは、その人の中での変化です。
左右差についても、痛みの訴えと一致しない限り、それだけを理由に故障を予言させません。
...
スクロールして何枚も撮るのは手間です。減らす道を2つ用意しています。

- **フルページのスクリーンショット（PDF）をそのまま受け取ります。**
  iPhone は Safari の画面なら、撮った直後に「フルページ」を選ぶと
  スクロールした先まで1枚の PDF で保存できます。長い画面を1回で渡せる唯一の道です
- **縦に長い画像は、読める大きさに切り分けてから送ります。**
  長辺に合わせて縮めると、横幅が100px台まで潰れて数字が読めなくなるためです
  （`tilesFor`）。境目は少し重ねて切り、同じ行が2度出ても二重に数えないよう
  コーチに指示しています

> PDF はブラウザ側で縮小できないため、そのまま渡します（上限 2.5MB）。
...
└── lib/
    ...
    ├── fit.ts                 FIT を解く（上下動・接地時間・左右バランス・パワー）
...
704 件のテストが、痛み検知・走行指示の検査・目標からのペースとVDOTの導出・心拍ゾーンの計算・
...
- FIT の項目番号・単位・倍率を取り違えない／あり得ない値は捨てる（`tests/fit.test.ts`）
- 古い練習を後から取り込んでも、走った日の順に並ぶ（`tests/workout.test.ts`）
- 詳しい記録が来たら差し替える・その時も本人の言葉は残す（`tests/workout.test.ts`）
- zip を開ける・一括書き出しの写真や設定ファイルは開かない（`tests/zip.test.ts`）
- 縦に長い画像を読める大きさに切り分ける・端を落とさない（`tests/images.test.ts`）
- 推移のピッチが片脚の回転数のまま残らない（`tests/workout.test.ts`）
