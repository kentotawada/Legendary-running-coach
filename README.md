...
### Garmin ユーザーがいちばん詰まるところ

「Strava とつないだのに何も入らない」の原因は、ほぼ **Garmin と Strava のリンクがまだ**であることです。
そこで、アプリの中に手順を置いています（`src/components/StravaGuide.tsx`）。

- カルテ →「ランニングアプリ」→「Garmin の時計とつなぐ手順」からいつでも開けます
- **初回の取り込みが0件だった時は、その場で手順への導線を出します。**
  つないだ直後に何も起きないのが、いちばん諦めやすい瞬間なので
- 画面の表示名は Strava の版で変わるため、日本語と英語の両方を併記しています
- 取り込んだ記録の出どころが Garmin だと分かった時は「Garmin からの自動連携も確認できました」と返します。
...
587 件のテストが、痛み検知・走行指示の検査・目標からのペースとVDOTの導出・心拍ゾーンの計算・
